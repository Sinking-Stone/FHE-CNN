...
void print_parameters(const seal::SEALContext &context)
{
    ...
    std::cout << "|   CoeffModulus::MaxBitCount: " << CoeffModulus::MaxBitCount(context_data.parms().poly_modulus_degree()) << std::endl;

	pFile << "/" << std::endl;
    pFile << "| Encryption parameters :" << std::endl;
    pFile << "|   scheme: CKKS" << std::endl;
    pFile << "|   poly_modulus_degree: " << context_data.parms().poly_modulus_degree() << std::endl;
    pFile << "|   CoeffModulus::MaxBitCount: " << CoeffModulus::MaxBitCount(context_data.parms().poly_modulus_degree()) << std::endl;
	
    /*
    Print the size of the true (product) coefficient modulus.
	CoeffModulus::MaxBitCount应该大于等于context_data.total_coeff_modulus_bit_count()才能保证安全
    */
    std::cout << "|   coeff_modulus size: ";
    std::cout << context_data.total_coeff_modulus_bit_count() << " (";
	pFile << "|   coeff_modulus size: ";
    pFile << context_data.total_coeff_modulus_bit_count() << " (";
    auto coeff_modulus = context_data.parms().coeff_modulus();
    std::size_t coeff_modulus_size = coeff_modulus.size();
    for (std::size_t i = 0; i < coeff_modulus_size - 1; i++)
    {
        std::cout << coeff_modulus[i].bit_count() << " + ";
		pFile << coeff_modulus[i].bit_count() << " + ";
    }
    std::cout << coeff_modulus.back().bit_count();
    std::cout << ") bits" << std::endl;
	pFile << coeff_modulus.back().bit_count();
    pFile << ") bits" << std::endl;
	pFile << "\\" << std::endl;
...
void import_parameters_cifar10(vector<double> &linear_weight, vector<double> &linear_bias, vector<vector<double>> &conv_weight, vector<vector<double>> &bn_bias, vector<vector<double>> &bn_running_mean, vector<vector<double>> &bn_running_var, vector<vector<double>> &bn_weight, size_t layer_num, size_t end_num)
{
	...
	if(layer_num == 20) dir = "resnet20_new";		// 有用
	...
	//  卷积参数权重
	pFile << "conv_weight:" << endl;
	for(int i=0;i<conv_weight.size();++i){
		for(int j=0;j<conv_weight[i].size();++j)
			pFile << conv_weight[i][j]<<  " ";
		pFile << endl;
	}
	...
	//  归一化参数权重
	pFile << "bn_bias:" << endl;
	for(int i=0;i<bn_bias.size();++i){
		for(int j=0;j<bn_bias[i].size();++j)
			pFile << bn_bias[i][j]<<  " ";
		pFile << endl;
	}
	pFile << "---------------------------"<<  endl;
	pFile << "bn_running_mean:" << endl;
	for(int i=0;i<bn_running_mean.size();++i){
		for(int j=0;j<bn_running_mean[i].size();++j)
			pFile << bn_running_mean[i][j]<<  " ";
		pFile << endl;
	}
	pFile << "---------------------------"<<  endl;
	pFile << "bn_running_var:" << endl;
	for(int i=0;i<bn_running_var.size();++i){
		for(int j=0;j<bn_running_var[i].size();++j)
			pFile << bn_running_var[i][j]<<  " ";
		pFile << endl;
	}
	pFile << "---------------------------"<<  endl;
	pFile << "bn_weight:" << endl;
	for(int i=0;i<bn_weight.size();++i){
		for(int j=0;j<bn_weight[i].size();++j)
			pFile << bn_weight[i][j]<<  " ";
		pFile << endl;
	}
	pFile << "---------------------------"<<  endl;
	...
	pFile << "linear_weight:" << endl;
	for(int i=0;i<linear_weight.size();++i){
		pFile << linear_weight[i] <<  " ";
	}
	pFile << endl;
	pFile << "---------------------------"<<  endl;
	pFile << "linear_bias:" << endl;
	for(int i=0;i<linear_bias.size();++i){
		pFile << linear_bias[i] <<  " ";
	}
	pFile << endl;
	pFile << "---------------------------"<<  endl;
...
pFile.open("./log.txt", ios::out|ios::app);
	...
	int boot_level = 14; // 进行14层进行自举
	int total_level = remaining_level + boot_level;

	vector<int> coeff_bit_vec;	// coeff_bit_vec: the bit-length of each level ：32
	...
	// 显示数据 0、17-31是51，1-16是46

	cout << "Setting Parameters" << endl;
	pFile<< "Setting Parameters" << endl;
	...
	double scale = pow(2.0, logp);	 // 设置比例大小derta
...
//	additional rotation kinds for CNN
	...
	pFile << "Generating Optimal Minimax Polynomials..." << endl;
	...
	pFile << "Adding Bootstrapping Keys..." << endl;
	...
	// 显示gal_steps_vector内容
	pFile<<"gal_steps_vector:\n";
	for(auto it=gal_steps_vector.begin();it!=gal_steps_vector.end();++it){
		std::cout<<*it<<" ";
	}
	pFile<<" \n";
	...
	pFile<<"________________"<<std::endl<<"create_galois_keys"<<std::endl;
	keygen.create_galois_keys(gal_steps_vector, gal_keys);
	pFile<<"________________"<<std::endl;

	bootstrapper_1.slot_vec.push_back(logn_1);
	bootstrapper_2.slot_vec.push_back(logn_2);
	bootstrapper_3.slot_vec.push_back(logn_3);

	// bootstrapping1 slot_vec
	for(auto it=bootstrapper_1.slot_vec.begin();it!=bootstrapper_1.slot_vec.end();++it){
		pFile<<"bootstrapper_1.slot_vec:"<<*it<<std::endl;
	}
	// bootstrapping2 slot_vec
	for(auto it=bootstrapper_2.slot_vec.begin();it!=bootstrapper_2.slot_vec.end();++it){
		pFile<<"bootstrapper_2.slot_vec:"<<*it<<std::endl;
	}
	// bootstrapping3 slot_vec
	for(auto it=bootstrapper_3.slot_vec.begin();it!=bootstrapper_3.slot_vec.end();++it){
		pFile<<"bootstrapper_3.slot_vec:"<<*it<<std::endl;
	}

	cout << "Generating Linear Transformation Coefficients..." << endl;
	pFile << "Generating Linear Transformation Coefficients..." << endl;
	...
	std::cout<<"start 64 thread to cnn"<<std::endl;
	pFile<<"start 64 thread to cnn"<<std::endl;
	...
	for(size_t image_id = start_image_id; image_id <=end_image_id; image_id++)
	{
		...
		pFile<<"image_id:"<<image_id<<std::endl;
		...
		import_parameters_cifar10(linear_weight, linear_bias, conv_weight, bn_bias, bn_running_mean, bn_running_var, bn_weight, layer_num, end_num);	// 加载权重

		std::cout<<"start pack images compactly"<<std::endl;pFile<<"start pack images compactly"<<std::endl;
		...
		std::cout<<"引入图像参数，并将图像归一化为-1~1...."<<std::endl;pFile<<"引入图像参数，并将图像归一化为-1~1：image num:"<<std::endl;
		...
		cout<<"图像标准答案：image_label: "<<image_label<<endl;
		pFile << "图像标准答案：image_label: " << image_label << std::endl;
		...
		pFile << "remaining level : " << context.get_context_data(cnn.cipher().parms_id())->chain_index() << endl;
		pFile << "scale: " << cnn.cipher().scale() << endl;
	...
	pFile << "all threads time : " << all_time_diff.count() / 1000 << " ms" << endl;	
	out_share << endl << "all threads time : " << all_time_diff.count() / 1000 << " ms" << endl;

}

void Adv_ResNet_cifar10_seal_sparse(size_t layer_num, size_t start_image_id, size_t end_image_id)
{
	// approximation boundary setting
	double B = 40.0;	// approximation boundary
	...
	std::cout<<"start 50 thread to cnn"<<std::endl;
	pFile<<"start 50 thread to cnn"<<std::endl;
	...
	for(size_t image_id = start_image_id; image_id <=end_image_id; image_id++)
	{
		...
		// 需要考虑st、co的大小！！！！！！！！！！！！！！！！！！
		multiplexed_parallel_batch_norm_seal_print(cnn, cnn, bn_bias[stage], bn_running_mean[stage], bn_running_var[stage], bn_weight[stage], epsilon, encoder, encryptor, evaluator, B, output, decryptor, context, stage);
		adv_Approx_ReLU_seal_print(cnn, cnn, comp_no, deg, alpha, tree, scaled_val, logp, encryptor, evaluator, decryptor, encoder, public_key, secret_key, relin_keys, B, output, context, gal_keys, stage);
		
		cout << "--------------------------------------------\nlayer 1\n--------------------------------------------\n" << endl;
		output << "--------------------------------------------\nlayer 1\n--------------------------------------------" << endl;
		pFile << "--------------------------------------------\nlayer 1\n--------------------------------------------\n" << endl;
		temp = cnn;stage = 1;st=1;co=16;
		multiplexed_parallel_convolution_print(cnn, cnn, co, st, fh, fw, conv_weight[stage], bn_running_var[stage], bn_weight[stage], epsilon, encoder, encryptor, evaluator, gal_keys, cipher_pool, output, decryptor, context, stage);
		multiplexed_parallel_batch_norm_seal_print(cnn, cnn, bn_bias[stage], bn_running_mean[stage], bn_running_var[stage], bn_weight[stage], epsilon, encoder, encryptor, evaluator, B, output, decryptor, context, stage);
		adv_Approx_ReLU_seal_print(cnn, cnn, comp_no, deg, alpha, tree, scaled_val, logp, encryptor, evaluator, decryptor, encoder, public_key, secret_key, relin_keys, B, output, context, gal_keys, stage);
		...
		relinetemp=temp.cipher();
		evaluator.rescale_to_next_inplace(relinetemp);
		temp = TensorCipher(temp.logn(), temp.k(), temp.h(), temp.w(), temp.c(), temp.t(), temp.p(), relinetemp);
		bootstrap_print(temp, temp, bootstrapper_2, output, decryptor, encoder, context, stage);
		cipher_add_seal_print(temp, cnn, cnn, evaluator, output, decryptor, encoder, context);
		adv_Approx_ReLU_seal_print(cnn, cnn, comp_no, deg, alpha, tree, scaled_val, logp, encryptor, evaluator, decryptor, encoder, public_key, secret_key, relin_keys, B, output, context, gal_keys, stage);
		...
		// evaluator.rescale_to_next_inplace(relinetemp);
		// temp = TensorCipher(temp.logn(), temp.k(), temp.h(), temp.w(), temp.c(), temp.t(), temp.p(), relinetemp);
		// bootstrap_print(temp, temp, bootstrapper_3, output, decryptor, encoder, context, stage);
		cipher_add_seal_print(temp, cnn, cnn, evaluator, output, decryptor, encoder, context);
		adv_Approx_ReLU_seal_print(cnn, cnn, comp_no, deg, alpha, tree, scaled_val, logp, encryptor, evaluator, decryptor, encoder, public_key, secret_key, relin_keys, B, output, context, gal_keys, stage);
		...
		multiplexed_parallel_batch_norm_seal_print(cnn, cnn, bn_bias[stage], bn_running_mean[stage], bn_running_var[stage], bn_weight[stage], epsilon, encoder, encryptor, evaluator, B, output, decryptor, context, stage);
		adv_Approx_ReLU_seal_print(cnn, cnn, comp_no, deg, alpha, tree, scaled_val, logp, encryptor, evaluator, decryptor, encoder, public_key, secret_key, relin_keys, B, output, context, gal_keys, stage);
		relinetemp=cnn.cipher();
		evaluator.rescale_to_next_inplace(relinetemp);
		cnn = TensorCipher(cnn.logn(), cnn.k(), cnn.h(), cnn.w(), cnn.c(), cnn.t(), cnn.p(), relinetemp);
...
// 更改relu为x^4
void Adv_ResNet_cifar10_seal_sparse1(size_t layer_num, size_t start_image_id, size_t end_image_id)
{
	// approximation boundary setting
	double B = 40.0;	// approximation boundary
	...
	for(size_t image_id = start_image_id; image_id <=end_image_id; image_id++)
	{
		...
		// 需要考虑st、co的大小！！！！！！！！！！！！！！！！！！
		multiplexed_parallel_batch_norm_seal_print(cnn, cnn, bn_bias[stage], bn_running_mean[stage], bn_running_var[stage], bn_weight[stage], epsilon, encoder, encryptor, evaluator, B, output, decryptor, context, stage);
		adv_Approx_ReLU1_seal_print(cnn, cnn, comp_no, deg, alpha, tree, scaled_val, logp, encryptor, evaluator, decryptor, encoder, public_key, secret_key, relin_keys, B, output, context, gal_keys, stage);
...
{
	cout<<"ResNet-32 for CIFAR-100 with SEAL and sparse input."<<endl;
	// approximation boundary setting
